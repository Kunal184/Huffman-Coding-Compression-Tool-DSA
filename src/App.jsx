...
const TableHeader = ({ children }) => (
  <th className="px-4 py-2 text-left text-xs font-bold uppercase tracking-wider text-huffman-accent border-b border-purple-200">
    {children}
  </th>
);
...
function App() {
  ...
  return (
    <div className="min-h-screen p-6 md:p-12 max-w-7xl mx-auto">
      {/* Header */}
      <header className="mb-12 border-b border-purple-200 pb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-6xl font-bold text-huffman-text">
            Huffman Visualizer
          </h1>
          <p className="mt-4 text-huffman-edge max-w-2xl text-sm leading-relaxed">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <label className="text-xs text-huffman-edge uppercase">Step-by-Step Discovery</label>
            <button 
              onClick={() => setShowSteps(!showSteps)}
              className={`relative inline-flex h-6 w-11 items-center transition-colors focus:outline-none border border-purple-300 ${showSteps ? 'bg-huffman-accent' : 'bg-purple-100'}`}
            >
              <span className={`inline-block h-4 w-4 transform bg-white transition-transform ${showSteps ? 'translate-x-6' : 'translate-x-1'}`} />
            </button>
          </div>
        </div>
      </header>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <section className="panel p-6">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
              <span className="w-2 h-2 bg-huffman-accent animate-pulse"></span>
              Input Buffer
            </h2>
          ...
          {result && (
            <section className="panel overflow-hidden">
              <div className="p-4 border-b border-purple-200 bg-purple-50">
                <h3 className="text-sm font-bold uppercase tracking-widest">Frequency Analysis</h3>
              </div>
                  ...
                  <tbody>
                    {Object.entries(result.frequencies).sort((a,b) => b[1] - a[1]).map(([char, freq]) => (
                      <tr key={char} className="hover:bg-purple-50 transition-colors">
                        ...
                        <td className="data-cell text-huffman-edge">{freq}</td>
                        <td className="data-cell font-bold text-huffman-text">{result.codes[char]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {result && (
            <section className="panel p-6">
              <h2 className="text-lg font-bold mb-6 border-b border-purple-200 pb-2">Codec Operations</h2>
              
              <div className="space-y-6">
                <div>
                  <label className="text-[10px] text-huffman-edge uppercase mb-2 block">Encoded stream (Bitstream)</label>
                  <div className="bg-purple-50 p-3 border border-purple-200 break-all text-xs text-huffman-accent leading-relaxed">
                    {result.encoded || '—'}
                  </div>
                </div>

                <div>
                  <label className="text-[10px] text-huffman-edge uppercase mb-2 block">Interactive Decoder</label>
                  <input 
                    type="text" 
                  ...
                  />
                  <div className="bg-purple-50 p-3 border border-purple-200 min-h-[40px] flex items-center">
                    <span className="text-huffman-edge text-[10px] mr-2">RESULT:</span>
                    <span className="text-huffman-text font-bold tracking-wider">{decodedText}</span>
                  </div>
                </div>
              </div>
            </section>
        ...
        <div className="lg:col-span-8 space-y-8">
            ...
            <section className="panel p-6 animate-fade-in">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-xs text-huffman-edge mt-1">
                ...
                <div className="flex gap-2 items-center">
                  <button 
                    ...
                    className={`px-4 py-2 font-bold flex items-center gap-2 transition-all border ${isPlaying ? 'bg-red-500/20 text-red-500 border-red-500/50' : 'bg-huffman-accent text-huffman-bg border-huffman-accent'}`}
                  >
                    {isPlaying ? (
                  ...
                  <div className="h-8 w-[1px] bg-purple-200 mx-1"></div>
                  <button 
                    disabled={currentStep === 0 || isPlaying}
                    onClick={() => setCurrentStep(Math.max(0, currentStep - 1))}
                    className="p-2 panel hover:bg-purple-100 disabled:opacity-30"
                    title="Previous Step"
                  >
                    ←
                  </button>
              ...
              <div className="bg-purple-50 p-4 text-sm border border-purple-200 mb-8 border-l-4 border-l-huffman-accent">
                {currentStepData?.description}
              </div>

              {/* Priority Queue (Heap) State Visualization */}
              <div className="mb-8">
                <h4 className="text-[10px] text-huffman-edge uppercase mb-4 tracking-widest">Priority Queue (Min-Heap Array)</h4>
                <div className="flex flex-wrap gap-3">
                  {currentStepData?.heapState.map((node, i) => (
                    <div key={`${i}-${node.id}`} className="group relative">
                      <div className="flex flex-col items-center justify-center w-14 h-14 panel bg-purple-50 group-hover:border-huffman-accent transition-all">
                      ...
                      <div className="absolute -top-1 -right-1 w-4 h-4 bg-huffman-accent text-white text-[10px] font-bold flex items-center justify-center">
                        {i}
                      </div>
                    </div>
                  ))}
                  {currentStepData?.heapState.length === 0 && (
                    <div className="text-huffman-edge italic text-sm">Heap is empty (Construction Complete)</div>
                  )}
                </div>
              </div>
            </section>
          ...
          <section className="panel p-6 h-[600px] flex flex-col relative overflow-hidden group">
            <div className="absolute top-6 left-6 z-10">
              <h3 className="text-xs font-bold uppercase tracking-widest text-huffman-edge">Prefix Free Binary Tree View</h3>
            </div>
            ...
            <div className="flex-grow w-full h-full cursor-grab active:cursor-grabbing overflow-auto">
              ...
              >
                ...
                {treeLayout.nodes.length === 0 && (
                  <text x="400" y="300" textAnchor="middle" className="fill-purple-200 italic text-xl">
                    Waiting for input...
                  </text>
                )}
              </svg>
            ...
            <div className="absolute bottom-6 left-6 flex gap-4 text-[10px] text-huffman-edge uppercase">
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 border border-huffman-accent bg-huffman-panel"></span>
                <span>Leaf (Symbol)</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 border border-huffman-edge/30"></span>
      ...
      {/* Footer Info */}
      <footer className="mt-12 pt-8 border-t border-purple-200 flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="text-[10px] text-huffman-edge uppercase tracking-[0.2em]">
          Algorithm: Huffman O(N log N) | Space: O(N) | Greedy Choice Property
        </div>
        <div className="flex gap-4">
          <span className="text-[10px] bg-purple-50 px-3 py-1 border border-purple-200 text-huffman-edge cursor-default">
